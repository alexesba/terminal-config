...
**Persistence:** `~/.local/state/gogh/current` is JSON with per-emulator theme entries (`alacritty`, `kitty`, `wezterm`) plus `last_active`. **`last_active` is not trusted alone for tmux hooks** — the outer host wins.
